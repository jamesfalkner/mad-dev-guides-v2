...
Open the `src/main/resources/application.properties` file. Add the following lines to configure the PostgreSQL datasource for Quarkus.
...
Open the `src/main/resources/import.sql` file.Add following lines to insert new inventory items to the database.
...
Once done, you can conveniently run your service using the Spring Boot maven plugin and test the endpoint.
...
More importantly, `Quarkus` offers a couple of features that can significantly speed up your inner loop development cycle:

* `Dev Mode`: This is a key aspect of the *Quarkus Developer Joy* philosophy. In Dev Mode, Quarkus automatically detects and applies code changes to your running application. This eliminates the need for the traditional _write-build-deploy-run_ loop. You can simply write your code, interact with the application, and see the changes reflected instantly. This reduces iteration time tremendously, allowing you to experiment and test ideas much faster.
* `Container-based Development`:  Quarkus applications are inherently container-friendly. This means you can leverage containerization to isolate dependencies and run mock instances of external services during development. This simplifies your development environment and avoids potential conflicts with system-wide dependencies.

By combining these features, Quarkus streamlines the inner loop process, allowing developers to focus on coding and iterating quickly without getting bogged down by build and deployment cycles.
