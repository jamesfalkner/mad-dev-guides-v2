...
Run the following https://quarkus.io/guides/cli-tooling[Quarkus CLI^] in the terminal.
...
image::innerloop/spring-boot-logo.png[Spring Boot, 600]
...
Go back to the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link']. Then, click on the `Open URL` icon of the Catalog Service.
...
image::innerloop/240px-NET_Core_Logo.png[.NET, 600]
...
`*Examine 'Startup.cs' class*` in the *$PROJECT_SOURCE/gateway-dotnet/* directory.
...
`*Examine 'ProductsController.cs' class*` in the *$PROJECT_SOURCE/gateway-dotnet/Controllers* directory.
...
In the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link'], from the *Developer view*,
...
*$PROJECT_SOURCE/gateway-dotnet/* directory.
...
In the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link'], `*click on 'Actions' -> 'Delete Pod' -> 'Delete'*`
...
First, scale down your _Inventory Service_ to 1 instance. In the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link'], from the *Developer view*,
...
Let's add the needed dependencies to *$PROJECT_SOURCE/inventory-quarkus/pom.xml*. 
In your {CHE_URL}[Workspace^, role='params-link'], `*edit the '$PROJECT_SOURCE/inventory-quarkus/pom.xml' file*`:
...
[source,shell,subs="{markup-in-source}",role=copypaste]
----
cd $PROJECT_SOURCE/inventory-quarkus
...
`*Generate traffic to Inventory Service*` and then, in the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link'], 
...
You can check this in the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link'], from the *Developer view*,
...
In the {openshift_cluster_console}/topology/ns/globex-innerloop-{user_name}?view=graph[OpenShift web console^, role='params-link'], `*click on '+Add' and select 'Database'*`
...
in *$PROJECT_SOURCE/inventory-quarkus/src/main/resources/application.properties* and rebuild the application.

In your {CHE_URL}[Workspace^, role='params-link'], `*edit the '$PROJECT_SOURCE/inventory-quarkus/pom.xml' file and add the
...
Then `*add the '%prod.quarkus.datasource.driver' parameter in 
 the '$PROJECT_SOURCE/inventory-quarkus/src/main/resources/application.properties' file*` as follows
