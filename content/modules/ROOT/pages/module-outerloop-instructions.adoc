...
You can use any Git server (e.g. GitHub, BitBucket, etc). For this lab we will use a Gitea git server.

Click on the `Developer Repository` button below.

image::outerloop/developer-repository-button.png[Developer Repository - Button, 300]

Then, click on `Sign In` and login via OpenShift as user{user_name}/{user_password}*`

image::outerloop/gitea-signin.png[Sign Up Gitea,700]

Create a new repository called *'inventory-quarkus'*.
click on the '+' icon -> '+ New Repository'*` as below and enter the following values:

image::outerloop/gitea-create-repository.png[Create New Repository,900]
...
git init
git remote add origin http://gitea-server.gitea.svc:3000/user{user_name}/inventory-quarkus.git
git add *
git commit -m "Initial"
git push http://user{user_name}:openshift@gitea-server.gitea.svc:3000/user{user_name}/inventory-quarkus.git
----
...
To http://gitea-server.gitea.svc:3000/user{user_name}/inventory-quarkus.git
 * [new branch]      master -> master
----

Once done, in your {GIT_URL}/user{user_name}/inventory-quarkus[Git Repository^, role='params-link'], *refresh the page of your inventory-quarkus repository*`. You should see the project files in the repository.

image::outerloop/cd-gitea-inventory-repo.png[Inventory Repository,900]
...
* url: `http://gitea-server.gitea.svc:3000/{user_name}/inventory-quarkus.git`
...
image::outerloop/openshift-inventory-pipeline-run.png[]
...
[source,bash]
----
--- Export Kubernetes resources for GitOps from my-project{user_name} to globex-outerloop-{user_name} ---
...
image::outerloop/che-gitops-export-result.png[Che - GitOps Export Result, 300]
...
Once done, in {GIT_URL}/user{user_name}/inventory-gitops[Git Repository^, role='params-link'], refresh the page of your `inventory-gitops repository`. You should see the files in the repository.
...
* Type: `git`
* Repository URL: `http://gitea-server.gitea.svc:3000/user{user_name}/inventory-gitops.git`
...
Once done, in your {GIT_URL}/user{user_name}[Git Repository^,role='params-link'], you should see the 3 new '*-gitops' repositories.
...
Argo CD has synchronized all the Inventory OpenShift manifests stored into your {GIT_URL}/user{user_name}/inventory-gitops[Inventory Repository^,role='params-link'] with your `globex-outerloop-{user_name}` project on OpenShift.
