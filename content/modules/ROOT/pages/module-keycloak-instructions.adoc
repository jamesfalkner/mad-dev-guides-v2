...
Back on the https://{console_url}/topology/ns/globex-keycloak-{user_name}?view=graph[OpenShift web console^] topology page, click the small "open" icon at the upper right of the Keycloak deployment:
...
* *Password*: The password you retrieved from the https://{console_url}/k8s/ns/globex-keycloak-{user_name}/secrets/keycloak-initial-admin[secret^] earlier.
...
[source,java,role="copypaste",subs="attributes"]
----
