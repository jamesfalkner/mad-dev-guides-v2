...
By creating an instance of a `Keycloak` object, the Keycloak operator will process it and instantiate a new Keycloak server. In the OpenShift console, click the `Add` icon, type `keycloak` into the search box, click on the "Keycloak" item, and click Create:
...
You should see `BUILD SUCCESS`. The new application will begin deploying. Wait for it to complete (dark blue circle for the `inventory` service on the OpenShift console).
